...
def _run_tests(
    ...
    *,
    ...
    # CI sets `MQT_WHEELHOUSE` to a directory of prebuilt wheels, along with a
    # `UV_FIND_LINKS` and a `UV_CONSTRAINT` pinning the exact version in it. Installing
    # from there skips a full C++ build in every single test job. `uv sync` always
    # installs the root project from the local tree -- no environment variable overrides
    # that -- so the project has to be excluded from the sync and installed separately.
